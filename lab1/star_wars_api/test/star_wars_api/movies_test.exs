defmodule StarWarsApi.MoviesTest do
  ...
  describe "movies" do
    ...
    test "create_movie/1 with valid data creates a movie" do
      valid_attrs = %{director: "some director", release_year: 42, title: "some title"}

      assert {:ok, %Movie{} = movie} = Movies.create_movie(valid_attrs)
      assert movie.director == "some director"
      assert movie.release_year == 42
    ...
    test "update_movie/2 with valid data updates the movie" do
      movie = movie_fixture()
      update_attrs = %{director: "some updated director", release_year: 43, title: "some updated title"}

      assert {:ok, %Movie{} = movie} = Movies.update_movie(movie, update_attrs)
      assert movie.director == "some updated director"
      assert movie.release_year == 43
